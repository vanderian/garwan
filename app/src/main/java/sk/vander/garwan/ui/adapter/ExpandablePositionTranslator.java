...
class ExpandablePositionTranslator {
    ...
    public void build(ExpandableSource source) {
        final int groupCount = source.getGroupCount();
        ...
        for (int i = 0; i < groupCount; i++) {
            final long groupId = source.getGroupId(i);
            final int childCount = source.getChildCount(i);
    ...
    public void restoreExpandedGroupItems(
            int[] restoreGroupIds,
            ExpandableSource source,
            PublishSubject<ExpandableAdapter.OnExpandEvent> event
//            todo observable
        ...
        for (int i = 0; i < restoreGroupIds.length; i++) {
            ...
            for (int j = index; j < idAndPos.length; j++) {
                ...
                if (id2 < id1) {
                    ...
                    if (event != null) {
                        event.onNext(ExpandableAdapter.OnExpandEvent.create(ExpandableAdapter.ExpandEvent.COLLAPSE, position, fromUser));
                    }

//                    if (source == null || source.onHookGroupCollapse(position, fromUser)) {
...
//                    }
                    ...
                    if (event != null) {
                        event.onNext(ExpandableAdapter.OnExpandEvent.create(ExpandableAdapter.ExpandEvent.EXPAND, position, fromUser));
                    }

//                    if (source == null || source.onHookGroupExpand(position, fromUser)) {
...
//        if (source != null || collapseListener != null) {
        if (source != null) {
...
//                }
    ...
    public long getExpandablePosition(int flatPosition) {
        if (flatPosition == RecyclerView.NO_POSITION) {
            return ExpandableHelper.NO_EXPANDABLE_POSITION;
        }
        ...
        long expandablePosition = ExpandableHelper.NO_EXPANDABLE_POSITION;
        ...
        for (int i = startIndex; i < groupCount; i++) {
            ...
            if (offset >= flatPosition) {
                // found (group item)
                expandablePosition = ExpandableHelper.getPackedPositionForGroup(i);
                break;
            } else {
                offset += 1;
            }
            ...
            if ((t & FLAG_EXPANDED) != 0) {
                ...
                if ((childCount > 0) && (offset + childCount - 1) >= flatPosition) {
                    // found (child item)
                    expandablePosition = ExpandableHelper.getPackedPositionForChild(i, (flatPosition - offset));
                    break;
                } else {
                    offset += childCount;
                }
    ...
    public int getFlatPosition(long packedPosition) {
        if (packedPosition == ExpandableHelper.NO_EXPANDABLE_POSITION) {
            return RecyclerView.NO_POSITION;
        }

        final int groupPosition = ExpandableHelper.getPackedPositionGroup(packedPosition);
        final int childPosition = ExpandableHelper.getPackedPositionChild(packedPosition);
        final int groupCount = mGroupCount;
